...
> Polar control plane (`https://wg.4950.store:2443`) 同时接受两种客户端：
...
```bash
curl -sSL https://wg.4950.store:2443/v1/install | \
...
1. 打开 wg-mac app
2. 切到 **Mesh Join** tab
3. Server URL 填 `https://wg.4950.store:2443`，token 粘贴 `polar_wg_…`
4. 点 **Join Mesh**
...
3. 服务器填 `https://wg.4950.store:2443`
4. Auth key 粘贴 `tskey-…`
...
| `tailscale up` 卡在 "waiting for login" | login-server URL 写错（trailing `/`、http vs https） | 严格用 `https://wg.4950.store:2443`（无尾斜杠） |
...
- [`doc/tailscale-custom-server-support.md`](./tailscale-custom-server-support.md) — 各平台 Tailscale 客户端如何切到 `https://wg.4950.store:2443` coordination server（含 macOS App Store 版的 CLI 切换、iOS/Android "Use alternate server" 入口位置、常见坑）
